...
>
> **状态**：方案已定稿（原两个待确认事项已决策，见第十一章「已决策事项」）。
...
**两层是 AND 关系，不是 OR**：工具权限是粗粒度总开关（"能不能用 Edit 工具"），文件 ACL 是细粒度路径覆盖层（"能写哪些路径"），**两层都通过才放行**。OR 关系会造成其中一层失效——允许 Edit 但路径被拒仍能改，或禁止 Edit 但路径允许仍能绕过。AND 关系保证管理员（配工具权限）与用户（配路径 ACL）各自独立配置、共同生效，互不越界。

白名单旁路只跳过第②层文件 ACL（因为这些工具不访问工作区文件、或在内部路径上操作），不跳过第①层工具权限（所有工具平等受工具权限管），也不跳过工作区隔离前缀（基础防线，无一例外）。
...
| 9 | 工具权限与文件 ACL 是 AND 关系 | 两层都通过才放行；OR 会导致一层失效。白名单工具跳过文件 ACL 但不跳过工具权限——所有工具平等受工具权限管 |
...
1. **第 1 步 Spike 先行**。决策：**Spike 先行**。理由：切面触发（注解扫描器是否绕过 Spring 代理）与异常透传（`ForbiddenException` 是否被框架转为 `isError=true` 而非 500）是整个方案的基石；若不成立需回退 JSON-RPC Filter 方案，而 Filter 方案会牵涉 `tools/list` 可见性裁剪，与「仅 tools/call 拦截、不动 tools/list」的约束有张力。Spike 成本低（加 aop + 最小切面）、收益高（去除最大不确定性），故实现步骤 1 必须先做。

2. **`default-policy` 默认值：allow-all**。决策：**allow-all**。理由：向后兼容（未映射用户行为不变）；与技术方案 5.3「业务路径走默认避免配置爆炸」一致；与 utils 应用端方案对齐。
